...
def update_sensitivity_analysis(revenue_change, margin_change, expense_change, multiple,
    ...
    adjusted_metrics_layout = html.Div([
        html.H3("Adjusted Valuation Metrics", style={'color': '#2c3e50', 'marginBottom': 20}),
        html.Div([
            # Original vs Adjusted comparison
            html.Table([
                html.Thead([
                    html.Tr([
                        html.Th("Metric", style={'padding': '12px', 'textAlign': 'left'}),
                        html.Th("Original", style={'padding': '12px', 'textAlign': 'right'}),
                        html.Th("Adjusted", style={'padding': '12px', 'textAlign': 'right'}),
                        html.Th("Change", style={'padding': '12px', 'textAlign': 'right'}),
                        html.Th("% Change", style={'padding': '12px', 'textAlign': 'right'}),
                    ], style={'backgroundColor': '#34495e', 'color': 'white'})
                ]),
                html.Tbody([
                    html.Tr([
                        html.Td("Net Patient Revenue", style={'padding': '10px'}),
                        html.Td(f"${base_revenue:,.0f}", style={'padding': '10px', 'textAlign': 'right'}),
                        html.Td(f"${adj_revenue:,.0f}", style={'padding': '10px', 'textAlign': 'right'}),
                        html.Td(f"${revenue_change_amt:+,.0f}",
                                style={'padding': '10px', 'textAlign': 'right',
                                       'color': '#27ae60' if revenue_change_amt >= 0 else '#e74c3c'}),
                        html.Td(f"{revenue_change:+.1f}%",
                                style={'padding': '10px', 'textAlign': 'right',
                                       'color': '#27ae60' if revenue_change >= 0 else '#e74c3c'}),
                    ], style={'backgroundColor': '#ecf0f1'}),
                    html.Tr([
                        html.Td("Operating Expenses", style={'padding': '10px'}),
                        html.Td(f"${base_expenses:,.0f}", style={'padding': '10px', 'textAlign': 'right'}),
                        html.Td(f"${adj_expenses:,.0f}", style={'padding': '10px', 'textAlign': 'right'}),
                        html.Td(f"${expense_change_amt:+,.0f}",
                                style={'padding': '10px', 'textAlign': 'right',
                                       'color': '#e74c3c' if expense_change_amt >= 0 else '#27ae60'}),
                        html.Td(f"{(expense_change_amt/base_expenses*100) if base_expenses != 0 else 0:+.1f}%",
                                style={'padding': '10px', 'textAlign': 'right',
                                       'color': '#e74c3c' if expense_change_amt >= 0 else '#27ae60'}),
                    ]),
                    html.Tr([
                        html.Td("Operating Income", style={'padding': '10px', 'fontWeight': 'bold'}),
                        ...
                        html.Td(f"${operating_income_change:+,.0f}",
                                style={'padding': '10px', 'textAlign': 'right', 'fontWeight': 'bold',
                                       'color': '#27ae60' if operating_income_change >= 0 else '#e74c3c'}),
                        html.Td(f"{(operating_income_change/base_operating_income*100) if base_operating_income != 0 else 0:+.1f}%",
                                style={'padding': '10px', 'textAlign': 'right', 'fontWeight': 'bold',
                                       'color': '#27ae60' if operating_income_change >= 0 else '#e74c3c'}),
                    ], style={'backgroundColor': '#ecf0f1'}),
                    html.Tr([
                        html.Td("EBITDA (Est.)", style={'padding': '10px', 'fontWeight': 'bold'}),
                        ...
                        html.Td(f"${ebitda_change:+,.0f}",
                                style={'padding': '10px', 'textAlign': 'right', 'fontWeight': 'bold',
                                       'color': '#27ae60' if ebitda_change >= 0 else '#e74c3c'}),
                        html.Td(f"{(ebitda_change/baseline.get('ebitda', 1)*100):+.1f}%",
                                style={'padding': '10px', 'textAlign': 'right', 'fontWeight': 'bold',
                                       'color': '#27ae60' if ebitda_change >= 0 else '#e74c3c'}),
                    ]),
                    html.Tr([
                        html.Td(f"Valuation ({multiple}x EBITDA)", style={'padding': '10px', 'fontWeight': 'bold',
                                                                           'fontSize': 16}),
                        ...
                        html.Td(f"${adj_valuation:,.0f}",
                                style={'padding': '10px', 'textAlign': 'right', 'fontWeight': 'bold',
                                       'fontSize': 16, 'color': '#e67e22'}),
                        html.Td(f"${valuation_change:+,.0f}",
                                style={'padding': '10px', 'textAlign': 'right', 'fontWeight': 'bold',
                                       'fontSize': 16, 'color': '#27ae60' if valuation_change >= 0 else '#e74c3c'}),
