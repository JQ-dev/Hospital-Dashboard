...
# Setup logging
logger = setup_logging('create_revenue', log_file='logs/create_revenue.log')
...
for year in years:
    try:
        ...
        # Filter to revenue worksheet (G200000) and exclude NULL worksheets
        alpha_df = alpha_df[(alpha_df.Worksheet == 'G200000') & (alpha_df.Worksheet.notna())]
        nmrc_df = nmrc_df[(nmrc_df.Worksheet == 'G200000') & (nmrc_df.Worksheet.notna())]
        ...
        col_names_file = get_col_names_file('revenue')
...
revenue = pd.concat(all_dfs, ignore_index=True)

logger.info(f"Total records before deduplication: {len(revenue):,}")
...
revenue['Process_Date'] = pd.to_datetime(revenue['Process_Date'], errors='coerce')
revenue['FY_End_dt'] = pd.to_datetime(revenue['FY_End'], errors='coerce')
revenue['Fiscal_Year_temp'] = revenue['FY_End_dt'].dt.year
...
revenue = revenue.drop('Fiscal_Year_temp', axis=1)
logger.info(f"Total records after deduplication: {len(revenue):,}")

# Create KPI column (strip prefixes like "PatRev " or "OpeExp ")
def clean_line_name(name):
    if pd.isna(name):
        return ''
    name = str(name).strip()
    if name.startswith('PatRev '):
        return name[7:].strip()
    elif name.startswith('OpeExp '):
        return name[7:].strip()
    else:
        return name

revenue['Item'] = revenue['Line_Name'].apply(clean_line_name).str.replace(' ', '_').str.replace(',', '').str.replace('(', '').str.replace(')', '').str.replace('"', '')
revenue['Fund'] = revenue['Column_name'].str.replace(' ', '_')
revenue['KPI'] = revenue['Fund'] + '_' + revenue['Item']
...
long_df = revenue[[
    ...
    # CMS Structure fields from Names_3_Rev.csv
    'Worksheet', 'Line', 'Column',
    'Line_Name', 'Column_name',
    'REV_GROUP', 'REV_SUBGROUP', 'REV_SUBGROUP.1', 'REVENUE_CENTER'
]].copy()
...
long_df = long_df.rename(columns={
    'year': 'Year',
    'KPI': 'Account_Name',
    'Line_Name': 'Revenue_Line_Name',
    'Column_name': 'Revenue_Column_Name',
    'REV_GROUP': 'Revenue_Group',
    'REV_SUBGROUP': 'Revenue_Subgroup',
    'REV_SUBGROUP.1': 'Revenue_Subgroup_Detail',
...
try:
    ...
    logger.info(f"Long-format revenue table created and saved to {output_dir}")
